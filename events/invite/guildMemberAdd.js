...
client.on("guildMemberAdd", async (member) => {
    ...
    //insivisible color hex is 2F3136
    let color = "2F3136"

if(db.get(`joinmessageembed_${member.guild.id}`) !== null) {
  let welcomeChannel = await client.channels.fetch(db.get(`joinchannelmessage_${member.guild.id}`))
  if(member.user.bot && welcomeChannel) return welcomeChannel.send( { embeds : [new MessageEmbed().setColor(color).setDescription(`Le bot ${member.toString()} nous a rejoint en utilisant ***l'api OAuth2***`)]})
  ...
  if(!usedInvite) return welcomeChannel.send({embeds : [new MessageEmbed().setColor(color).setDescription(`**Je n'arrive pas à trouver** comment ${member.toString()} a rejoint le serveur.`)]}).catch(err => {});
  if(usedInvite.code === member.guild.vanityURLCode) {
     if(welcomeChannel) welcomeChannel.send({embeds : [new MessageEmbed().setColor(color).setDescription(`${member.toString()} nous a rejoint en utilisant ***le lien d'invitation personnalisé du serveur.***`)]});
...
if(welcomeChannel) welcomeChannel.send({embeds: [embedj]}).then((msggg)=> {
