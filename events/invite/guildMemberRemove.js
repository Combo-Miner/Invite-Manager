...
client.on("guildMemberRemove", async (member) => {
   ...
   if(member.partial) member = await member.fetch();
    ...
    if(db.get(`leavemessageembed_${member.guild.id}`) !== null) {
      ...
      if(member.user.bot && welcomeChannel) return welcomeChannel.send({ embeds : [new MessageEmbed().setColor(color).setDescription(`Le bot ${member.toString()} nous a quitter, il avait rejoins en utilisant ***l'api OAuth2***`)]})
      let user = db.get(`inviter_${member.guild.id}_${member.id}`);
      if(user === null&& welcomeChannel) return welcomeChannel.send({embeds : [new MessageEmbed().setColor(color).setDescription(`**${member.user.tag}** a quitter le serveur, mais **je n'arrive pas à trouver** comment il l'avait rejoint.`)]});
      if(user === 'vanity'&& welcomeChannel) return welcomeChannel.send({embeds : [new MessageEmbed().setColor(color).setDescription(`**${member.user.tag}** nous a quitter il avais rejoint en utilisant ***le lien d'invitation personnalisé du serveur.**`)]});
      let iv2 = await client.users.fetch(user);
  ...
  if(invite !== m.ID.split('_')[3] && member.roles.cache.has(m.ID.split('_')[2])) {
    member.roles.remove(m.ID.split('_')[2]).catch()
      ...
      if(welcomeChannel) welcomeChannel.send({embeds: [embedj]})
   } else {
      let welcomeChannel = await client.channels.fetch(db.get(`leavechannelmessage_${member.guild.id}`)).catch(err => {});
